...
func SendOTP(targetEmail, otp string) error {
	...
	mail, err := config.GetMailConfig()
	if err != nil {
		log.Fatal(err)
	}

	app, err := config.GetServerConfig()
	if err != nil {
		log.Fatal(err)
	}
	...
	// Create new message
	m := gomail.NewMessage()
	m.SetHeader("From", app.AppName+" <"+mail.Address+">")
...
<body>
	...
	// Create dialer
	d := gomail.NewDialer(mail.Mailer, mail.Port, mail.Username, mail.Password)
