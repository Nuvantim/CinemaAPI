...
SELECT st.id AS showtime_id, se.id AS seat_id, (st.base_price * se.seat_price_modifier) AS ticket_price FROM public.showtime st JOIN public.seat se ON st.screen_id = se.screen_id WHERE st.id = $1 AND se.id = $2;
